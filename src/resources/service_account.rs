...
impl ServiceAccount {
    pub(crate) fn get() -> Self {
        dotenv::dotenv().ok();
        let credential_json = std::env::var("GOOGLE_APPLICATION_CREDENTIALS")
            .expect("GOOGLE_APPLICATION_CREDENTIALS environment parameter required");
        let account: Self =
            serde_json::from_str(&credential_json).expect("serivce account file not valid");
